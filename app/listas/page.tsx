...
export default function ListasPage() {
  ...
  const processExcelFile = (file: File, fileType: "excel1" | "excel2") => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Extraer datos desde las columnas B y E
        const extractedData = extractDataFromExcel(worksheet)
        ...
        alert("Error al procesar el archivo Excel: " + (error as Error).message)
  ...
  const extractDataFromExcel = (worksheet: XLSX.WorkSheet): string[] => {
    const data: string[] = []

    // Obtener el rango del worksheet
    const range = XLSX.utils.decode_range(worksheet["!ref"] || "A1")

    // Empezar desde la fila 0 y continuar hasta el final
    for (let row = 0; row <= range.e.r; row++) {
      const cellB = worksheet[XLSX.utils.encode_cell({ r: row, c: 1 })] // Columna B (índice 1)
      const cellE = worksheet[XLSX.utils.encode_cell({ r: row, c: 4 })] // Columna E (índice 4)

      // Verificar si las celdas existen y tienen valores
      if (cellB && cellB.v !== undefined) {
        const productName = String(cellB.v).trim()

        // Si hay precio en columna E, combinarlo
        if (cellE && cellE.v !== undefined) {
          const price = String(cellE.v).trim()
          if (price) {
            // Verificar si NO es una subcategoría de vinos
            if (!isWineSubcategoryTitle(productName)) {
              data.push(`${productName} $${price}`)
            }
          }
        } else {
          // Si no hay precio, es probablemente una categoría
          // Verificar si NO es una subcategoría de vinos
          if (!isWineSubcategoryTitle(productName)) {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>, fileType: "excel1" | "excel2") => {
    ...
    if (file) {
      processExcelFile(file, fileType)
    }
  }
